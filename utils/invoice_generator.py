...
class InvoiceGenerator:
    ...
    def _add_header(self):
        ...
        self.story.append(Spacer(1, 0.2*inch))  # Reduced from 0.3*inch
    ...
    def _add_customer_and_invoice_info(self):
        ...
        self.story.append(Spacer(1, 0.2*inch))  # Reduced spacing
    ...
    def _add_order_items(self):
        ...
        items_style = TableStyle([
            ...
            # Item rows
            ('FONTNAME', (0, 1), (-1, len(items_data)-5), 'Helvetica'),
            ('FONTSIZE', (0, 1), (-1, len(items_data)-5), 9),
            ('VALIGN', (0, 1), (-1, -1), 'TOP'),
            ('GRID', (0, 0), (-1, len(items_data)-5), 0.5, colors.grey),
            
            # Summary rows
            ('FONTNAME', (2, len(items_data)-4), (3, -1), 'Helvetica-Bold'),
            ('FONTSIZE', (2, len(items_data)-4), (3, -1), 10),
            ('ALIGN', (2, len(items_data)-4), (3, -1), 'RIGHT'),
        ...
        items_table = Table(items_data, colWidths=[3*inch, 0.5*inch, 1.25*inch, 1.25*inch])
    ...
    def _add_delivery_info(self):
        ...
        if self.order.delivery_type == 'delivery':
            delivery_info = f"Delivery Type: {delivery_type_display}\nDelivery Address: {self.order.delivery_address}\nPhone: {self.order.phone_number}"
        else:
            delivery_info = f"Delivery Type: {delivery_type_display}\nPhone: {self.order.phone_number}"
        
        if self.order.special_instructions:
            delivery_info += f"\nSpecial Instructions: {self.order.special_instructions}"
    ...
    def _add_footer(self):
        ...
        <br/>
        <i>Thank you for your order! Hope you enjoy our bakery products.</i>
        """
